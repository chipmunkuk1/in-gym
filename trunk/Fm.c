...
/* return : envelope output */
 signed int FM_CALC_SLOT( FM_SLOT *SLOT )
{
...
/* set total level */
 void set_tl(FM_CH *CH,FM_SLOT *SLOT , int v,int csmflag)
{
    v &= 0x7f;
    v = (v<<7)|v; /* 7bit -> 14bit */
    SLOT->TL = (v*EG_ENT)>>14;
...
/* set attack rate & key scale  */
 void set_ar_ksr(FM_CH *CH,FM_SLOT *SLOT,int v,signed int *ar_table)
{
    SLOT->KSR  = 3-(v>>6);
...
/* set decay rate */
 void set_dr(FM_SLOT *SLOT,int v,signed int *dr_table)
{
...
/* set sustain rate */
 void set_sr(FM_SLOT *SLOT,int v,signed int *dr_table)
{
...
/* set release rate */
 void fm_set_sl_rr(FM_SLOT *SLOT,int v,signed int *dr_table)
{
...
/* ---------- frequency counter for operater update ---------- */
 void fm_CALC_FCSLOT(FM_SLOT *SLOT , int fc , int kc )
{
    int ksr;
...
/* ---------- frequency counter  ---------- */
 void CALC_FCOUNT(FM_CH *CH )
{
    if( CH->SLOT[SLOT1].Incr==-1){
        int fc = CH->fc;
        int kc = CH->kcode;
        fm_CALC_FCSLOT(&CH->SLOT[SLOT1] , fc , kc );
        fm_CALC_FCSLOT(&CH->SLOT[SLOT2] , fc , kc );
        fm_CALC_FCSLOT(&CH->SLOT[SLOT3] , fc , kc );
        fm_CALC_FCSLOT(&CH->SLOT[SLOT4] , fc , kc );
    }
}

/* ---------- frequency counter  ---------- */
 void OPM_CALC_FCOUNT(YM2151F *OPM , FM_CH *CH )
{
    if( CH->SLOT[SLOT1].Incr==-1)
    {
        int fc = CH->fc;
        int kc = CH->kcode;
        fm_CALC_FCSLOT(&CH->SLOT[SLOT1] , OPM->KC_TABLE[fc + CH->SLOT[SLOT1].DT2] , kc );
        fm_CALC_FCSLOT(&CH->SLOT[SLOT2] , OPM->KC_TABLE[fc + CH->SLOT[SLOT2].DT2] , kc );
        fm_CALC_FCSLOT(&CH->SLOT[SLOT3] , OPM->KC_TABLE[fc + CH->SLOT[SLOT3].DT2] , kc );
        fm_CALC_FCSLOT(&CH->SLOT[SLOT4] , OPM->KC_TABLE[fc + CH->SLOT[SLOT4].DT2] , kc );
    }
}
...
static int FMInitTable( void )
{
	...
	pYMTable=GlobalAlloc(GPTR, TL_MAX*2*sizeof(int));
    TL_TABLE = (int*)GlobalLock(pYMTable);
...
/* OPN/OPM Mode  Register Write */
 void FMSetMode( FM_ST *ST ,int n,int v )
{
    /* b7 = CSM MODE */
    /* b6 = 3 slot mode */
    /* b5 = reset b */
...
/* ---------- calcrate timer A ---------- */
 void CALC_TIMER_A( FM_ST *ST , FM_CH *CSM_CH ){
...
/* ---------- calcrate timer B ---------- */
 void CALC_TIMER_B( FM_ST *ST,int step){
...
static void OPNWriteReg(FM_OPN *OPN, int r, int v)
{
    ...
    case 0x70:  /*     SR */
        ...
        break;
    case 0x80:  /* SL, RR */
        fm_set_sl_rr(SLOT,v,OPN->ST.DR_TABLE);
        break;
    case 0x90:  /* SSG-EG */
#ifndef SEG_SUPPORT
...
void YM2203UpdateOne(int num, void *buffer, int length)
{
    ...
    if( (State->mode & 0xc0) ){
        ...
        if( cch[2]->SLOT[SLOT1].Incr==-1){
            /* 3 slot mode */
            fm_CALC_FCSLOT(&cch[2]->SLOT[SLOT1] , OPN->SL3.fc[1] , OPN->SL3.kcode[1] );
            fm_CALC_FCSLOT(&cch[2]->SLOT[SLOT2] , OPN->SL3.fc[2] , OPN->SL3.kcode[2] );
            fm_CALC_FCSLOT(&cch[2]->SLOT[SLOT3] , OPN->SL3.fc[0] , OPN->SL3.kcode[0] );
            fm_CALC_FCSLOT(&cch[2]->SLOT[SLOT4] , cch[2]->fc , cch[2]->kcode );
        }
...
/* ---------- update one of chip ----------- */
void YM2612UpdateOne(int num, void **buffer, int length, BOOL mJAZZ)
{
...
#else
        /* stereo separate */
        bufL[i] += ((dataL>>OPN_OUTSB)*3)>>(mJAZZ?3:2); // Dave: adds it in 3/4
        bufR[i] += ((dataR>>OPN_OUTSB)*3)>>(mJAZZ?3:2);

//		bufL[i] += (dataL*3)>>(OPN_OUTSB+1);
//		bufR[i] += (dataR*3)>>(OPN_OUTSB+1);

//		if (bufL[i]>0x7fff) bufL[i]=0x7fff; if (bufL[i]<-0x8000) bufL[i]=-0x8000;
//		if (bufR[i]>0x7fff) bufR[i]=0x7fff; if (bufR[i]<-0x8000) bufR[i]=-0x8000;
#endif
               ...
               FM_TIMERHANDLER TimerHandler,FM_IRQHANDLER IRQHandler)
{
	...
	pYMChip=GlobalAlloc(GPTR, sizeof(YM2612) * FMNumChips);

    if( (FM2612 = (YM2612 *)GlobalLock(pYMChip))==NULL)
        return (-1);
    /* clear */
