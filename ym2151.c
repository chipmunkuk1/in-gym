...
int YM2151Init(int num, int clock, int rate)
{
	...
	YMPSGp = GlobalAlloc(GPTR, sizeof(YM2151) * YMNumChips);
	YMPSG = (YM2151 *)GlobalLock(YMPSGp);
...
INLINE signed int ym2151_op_calc(YM2151Operator * OP, unsigned int env, signed int pm)
{
	UINT32 p;
...
INLINE signed int ym2151_op_calc1(YM2151Operator * OP, unsigned int env, signed int pm)
{
	UINT32 p;
	INT32  i;
...
INLINE void ym2151_chan_calc(unsigned int chan)
{
	YM2151Operator *op;
	unsigned int env;
	UINT32 AM = 0;
	...
	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */
	...
	if (env < ENV_QUIET)
		*(op+1)->connect += ym2151_op_calc(op+1, env, m2);

	env = volume_calc(op+2);	/* C1 */
	if (env < ENV_QUIET)
		*(op+2)->connect += ym2151_op_calc(op+2, env, c1);

	env = volume_calc(op+3);	/* C2 */
	if (env < ENV_QUIET)
		chanout[chan]    += ym2151_op_calc(op+3, env, c2);
...
INLINE void chan7_calc(void)
{
	...
	*op->mem_connect = op->mem_value;	/* restore delayed sample (MEM) value to m2 or c2 */
	...
	{
		if (env < ENV_QUIET)
			chanout[7] += ym2151_op_calc(op+3, env, c2);
	}
	/* M1 */
	op->mem_value = mem;
}
...
INLINE void ym2151_advance_eg(void)
{
	YM2151Operator *op;
	unsigned int i;
...
INLINE void ym2151_advance(void)
{
	YM2151Operator *op;
	unsigned int i;
	int a,p;
